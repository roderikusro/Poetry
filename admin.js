...
function addStanzaField(value = '', timestamp = 0) {
  stanzaCount++;
  ...
  div.innerHTML = `
    <div class="stanza-field-header">
      ...
      <div style="display:flex; align-items:center; gap: 8px;">
        <span style="font-size: 0.8rem; opacity: 0.7;">Sync:</span>
        <input type="number" class="admin-input stanza-time-input" placeholder="Detik (misal: 15)" value="${timestamp || ''}" style="width: 100px; padding: 4px 8px; min-height: unset; height: 32px;" title="Detik ke berapa bait ini muncul">
...
function savePoem() {
  ...
  // Collect stanzas and timestamps
  const stanzas = [];
  const timestamps = [];
  document.querySelectorAll('.admin-stanza-field').forEach(field => {
    ...
    if (val && val !== '<br>' && val !== '<div><br></div>') {
      stanzas.push(val);
      timestamps.push(parseInt(timeInput.value) || 0);
    }
  });
